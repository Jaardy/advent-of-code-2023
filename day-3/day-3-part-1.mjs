...
function getNumberIfSymbolAdjacent([num, startIndex, endIndex, rowIndex]) {
  ...
  if (input[rowIndex][startIndex - 1])
    borderValues.push(input[rowIndex][startIndex - 1]);
  if (input[rowIndex][endIndex + 1])
    borderValues.push(input[rowIndex][endIndex + 1]);

  return !borderValues
    .filter((v) => v)
    .every((value) => value === "." || value.match(/\d+/))
    ? +num
    : 0;
}
...
console.log(
  partNumbers
    .map((numberData) => getNumberIfSymbolAdjacent(numberData))
